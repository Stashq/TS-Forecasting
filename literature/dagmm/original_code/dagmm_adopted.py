...
class DAGMM(Algorithm, PyTorchUtils):
    ...
    def __init__(self, n_features: int = 1, hidden_size: int = 5,
                 lambda_energy=0.1, lambda_cov_diag=0.005, gmm_k=3,
                 normal_percentile=80, sequence_length=30,
                 autoencoder_type=AutoEncoderModule, autoencoder_args=None,
        ...
        # Up to which percentile data should be considered normal
        ...
        autoencoder = self.autoencoder_type(
            n_features, hidden_size=self.hidden_size, **self.autoencoder_args)
        self.dagmm = DAGMMModule(
            autoencoder, n_gmm=self.gmm_k, latent_dim=self.hidden_size + 2,
    ...
    def fit(self, X: pd.DataFrame):
        ...
        summary_writer = SummaryWriter(log_dir="./logs_/dagmm2")
        tag = "train_loss"  # + datetime.now().strftime("%Y-%m-%d_%H:%M:%S")

        for epoch in trange(self.num_epochs):
            loss = 0
            for input_data in data_loader:
                input_data = self.to_var(input_data)
                total_loss, _, _, _ = self.dagmm_step(input_data.float())
                loss += total_loss
            summary_writer.add_scalar(
                tag, loss/len(data_loader), epoch)
