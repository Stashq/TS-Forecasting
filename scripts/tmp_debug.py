...
def find_th(
    ...
    for ds_id in ds_ids:
        ...
        scaler = exp.datasets_params.loc[ds_id]['scaler']
        ...
        train_ds = get_dataset(
            ...
            window_size=window_size, ts_scaler=scaler)
        test_ds = get_dataset(
        ...
        # print('Computing a_scores...')
        # exp_a_scores = a_score_exp(
        #     train_dl, test_dl, exp, calculate_training_a_score=False,
        #     scale=False, models_ids=m_ids)
        # train_a_scores, test_a_scores = exctract_a_scores(
        #     [exp_a_scores]  # , train_len=train_ds.n_points, test_len=test_ds.n_points
        # )
        ...
        for m_id in m_ids:
            ...
            dirpath = f'notebook_a_scores/{collection_name}/{ds_name}/{m_name}/{model_train_date}/'
            save_th_exp(std_th_df, dirpath + 'std_th_exp.csv')
...
find_th(
    ...
    ths=np.linspace(4e-6, 4e-4, 100),
    load_scores=False, save_scores=True,
    ds_ids=[0, 2, 6], m_ids=[2], scale=False,
    calculate_training_a_score=False
)