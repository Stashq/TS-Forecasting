...
models_params = [
    # ModelParams(
    #     name_=f'ConvAE_ws{window_size}_nk10_ks5_es50', cls_=ConvAE,
    #     init_params=dict(
    #         window_size=window_size, c_in=c_in, n_kernels=10,
    #         kernel_size=5, emb_size=10),
    #     WrapperCls=ConvAEWrapper
    # ),
    # ModelParams(
    #     name_=f'ConvAE_ws{window_size}_nk20_ks5_es50', cls_=ConvAE,
    #     init_params=dict(
    #         window_size=window_size, c_in=c_in, n_kernels=20,
    #         kernel_size=5, emb_size=50),
    #     WrapperCls=ConvAEWrapper
    # ),
    ...
    # ModelParams(
    #     name_="VELC_h50_l2_z10_N30_th0.025", cls_=VELC,
    #     init_params=dict(
    #         c_in=c_in, window_size=window_size, h_size=50, n_layers=2, z_size=10,
    #         h1_size=1024, h2_size=512,
    #         N_constraint=30, threshold=0.025),
    #     WrapperCls=VELCWrapper
    # ),
    # ModelParams(
    #     name_="VELC_h100_l2_z50_N50_th0.025", cls_=VELC,
    #     init_params=dict(
    #         c_in=c_in, window_size=window_size, h_size=100, n_layers=2, z_size=50,
    #         h1_size=1024, h2_size=512,
    #         N_constraint=50, threshold=0.025),
    #     WrapperCls=VELCWrapper
    # ),

    # ModelParams(
    #     name_="TadGAN_h50_l1_z10", cls_=TADGAN,
    #     init_params=dict(
    ...
    # ),
    # ModelParams(
    #     name_="TadGAN_h100_l2_z50", cls_=TADGAN,
    #     init_params=dict(
    ...
    # ),
    # ModelParams(
    #     name_="AnomTrans_l2_d128_lambda10", cls_=AnomalyTransformer,
    #     init_params=dict(
    #         window_size=window_size, c_in=c_in, d_model=128, n_layers=2,
    #         lambda_=10),
    #     WrapperCls=ATWrapper),
    # ModelParams(
    #     name_="AnomTrans_l3_d128_lambda3", cls_=AnomalyTransformer,
    #     init_params=dict(
    #         window_size=window_size, c_in=c_in, d_model=128, n_layers=3,
    #         lambda_=3),
    #     WrapperCls=ATWrapper),
    # ModelParams(
...
for ds_name, models_info in models_infos.items():
    for m_name, exp_date in models_info.items():
        exp = load_experimentator(
            f'./saved_experiments/{exp_date}.pkl')
        m_ids = exp.models_params[
            exp.models_params['name_'] == m_name
        ].index.to_list()
        find_th(
            exp, ws_list=[50, 100, 200, 300, 400, 500],
            ths=np.linspace(5e-6, 2e-2, 1000),
